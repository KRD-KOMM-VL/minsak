...
function initDatepicker(){
	...
	jQuery('.datepicker').datepicker({
		showOn: "button",
		buttonImage: "/static/images/ico-calendar.gif",
		dateFormat: 'dd/mm/y',
...
function initPopups() {
	...
	function positionLightbox() {
		if(_lightbox) {
			...
			// horizontal position
			if (_lightbox.hasClass('lightbox2')) {
                if (_fader.width() > _lightbox.outerWidth()) _lightbox.css({left:_fader.width() / 2});
	...
	jQuery('.lightbox.static').each(function() {
	    _lightbox = jQuery(this);
	    _lightbox.css('margin-left', 0);
	    toggleState(true);
	});
	// lightbox openers
	...
	jQuery(window).scroll(positionLightbox);
    	jQuery(document).keydown(function (e) {
    	    if (_lightbox != null && !_lightbox.hasClass('unclosable')) {
...
//init class if js work
function initClass() {document.getElementsByTagName('body')[0].className = "js-active";}
